...
It also exposed a cross-tool footgun: a manual resume reads the stash without consuming it, so the still-fresh file remained armed for any fresh Claude session on the same machine (60-minute window). Fixed in the preamble: a reader who gets the stash via the manual fallback is instructed to move it to respawn-last.md before resuming. Re-verified live the same day: after `/new`, the resumed Codex agent opened with "Resumed from respawn.", gave an accurate done/open summary, moved the stash to respawn-last.md per the preamble, and reported doing so. The full Claude-authored-spec, Codex-executed round trip passes.
